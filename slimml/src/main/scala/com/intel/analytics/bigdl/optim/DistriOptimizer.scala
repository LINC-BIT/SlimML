...
object DistriOptimizer extends AbstractOptimizer {
  ...
  private[optim] def optimize[T: ClassTag](
    ...
    val notEpoch = state.get[Int]("notEpoch").get
    val useComp = state.get[Boolean]("useComp").get
    val onlyComp = state.get[Boolean]("onlyComp").get
    val getGradient = state.get[Boolean]("getGradient").get

    var compMeanAry = new Array[Float](_subModelNumber)
    var trainMeanAry = new Array[Float](_subModelNumber)
    ...
    while (!endWhen(driverState)) {
      ...
      val compMeanSum = sc.accumulator(0.0f,"compMean")
      val trainMeanSum = sc.accumulator(0.0f,"trainMean")
      for(u <- 0 until _subModelNumber){
        compMeanAry(u) = 0.0f
        trainMeanAry(u) = 0.0f
      }
      val compTimeCost = sc.accumulator(0L,"comp time")
      val trainTimeCost = sc.accumulator(0L,"train time")
      ...
      val numFinishedModelUpdates: Int = dataRDD
        .zipPartitions(models, preservesPartitioning = true) { (data, modelIter) => {
          ...
          val selIdx = ArrayBuffer[(Int,Int)]()
          if(getGradient == true && curEpoch > notEpoch) {
            val startGetGradient = System.nanoTime()
            val gradientThreads = Engine.default.invokeAndWait2((0 until _subModelNumber).map(i =>
              () => {
                ...
                val grad = Tensor[T]()
                val compGradSum = Tensor()
                for (cur <- 1 to bs) {
                  val preGrad = localModel.getParametersTable()[Table](layerName)[Tensor[T]](gradName)
                  if (cur == 1) grad.resizeAs(preGrad).copy(preGrad)
                  else grad.copy(preGrad)
                  val oInput = if(useComp == false) input.narrow(1, cur, 1)
                  else input.narrow(1, cur, 1).narrow(2,1,featureSize)
                  val oTarget = if(useComp == false) target.narrow(1, cur, 1)
                  else target.narrow(1,cur,1).narrow(2,1,1)
                  ...
                  grad.sub(trainGrad)
                  if(cur == 1) compGradSum.resizeAs(grad).copy(grad)
                  else compGradSum.add(grad)
                  val gsum = grad.abs().sum().asInstanceOf[Float] // get gradient
                  gradInfo(i) += Tuple3(i, cur, if (gsum.isInfinite || gsum.isNaN) 0.0 else gsum)
                }
                compMeanAry(i) = compGradSum.abs().sum().asInstanceOf[Float] / bs
                i
              }
            ), timeout)
            ...
            // filter
            val mergedGrad = gradInfo.reduce(_ ++ _)

            if (notIteration > 0 && curIteration % notIteration == 0) gradInfoSum += mergedGrad.map(_._3) // get gradient of this iteration
            ...
            if(taskStrategy == "meandiv10"){
              ...
              for (u <- 0 until gradNum) {
                if (mergedGrad(u)._3 >= (mgMean * taskRatio)) {
                  selIdx += Tuple2(mergedGrad(u)._1 ,mergedGrad(u)._2)
                }
                else {
            ...
            else if(taskStrategy == "fixedratio"){
              val sortedG = mergedGrad.sortWith(_._3 < _._3)
              val stIdx = (gradNum * taskRatio).toInt
              iterationDelNum += stIdx
              for(u <- stIdx until(gradNum)){
                selIdx += Tuple2(sortedG(u)._1, sortedG(u)._2)
              }
            }
            else{
              val sortedG = mergedGrad.sortWith(_._3 < _._3)
            ...
            compTimeCost += (System.nanoTime() - startGetGradient)
          }

          val inputFeature = batch.getInput().toTensor
          val targetLabel = batch.getTarget().toTensor
          val stIdx = selIdx.sortWith(_._1 < _._1)
          ...
          val trainingThreads = Engine.default.invokeAndWait2((0 until _subModelNumber).map(i =>
            () => {
              ...
              val featureSize = inputFeature.size(2)/ targetLabel.size(2)
              val compOrigSize = targetLabel.size(2)
              val (inputXy,targetXy) = if(taskDel && curEpoch > notEpoch && getGradient && stIdx.length > 0){
                val inTaskSize = stIdx.length / _subModelNumber
                val inExtraTask = stIdx.length % _subModelNumber
                val offset = i * inTaskSize + math.min(i,inExtraTask)
                val length = inTaskSize + (if(i < inExtraTask) 1 else 0)
                val selInput = Tensor(length,inputFeature.size(2))
                val selTarget = Tensor(length,targetLabel.size(2))
                var currentIndex = 1
                for(cur <- offset  until (offset+length)){
                  selInput.update(currentIndex,inputFeature.narrow(1,stIdx(cur)._1 * stackSize + stIdx(cur)._2,1))
                  selTarget.update(currentIndex,targetLabel.narrow(1,stIdx(cur)._1 * stackSize + stIdx(cur)._2,1))
                  currentIndex += 1
                }
                (selInput,selTarget)
              }
              else{
                (miniBatchBuffer(i).getInput().toTensor,miniBatchBuffer(i).getTarget().toTensor)
              }

              val (input,target) = if(!useComp){ (inputXy,targetXy) }
              else{
                if(onlyComp){ (inputXy.narrow(2,1,featureSize),targetXy.narrow(2,1,1)) }
                else{
                  val bs = inputXy.size(1)
                  (inputXy.narrow(2,featureSize+1,(compOrigSize-1)*featureSize)
                    .resize(bs*(compOrigSize-1),featureSize),
                    targetXy.narrow(2,2,compOrigSize-1)
                      .resize(bs*(compOrigSize-1),1))
                }
              }

              if(input.size(1) > 0){
                val bs = input.size(1)
                val preGrad = localModel.getParametersTable()[Table](layerName)[Tensor[T]](gradName).clone()
                val output = localModel.forward(input)
                lossArray(i) = ev.toType[Double](localCriterion.forward(output, target))
                val errors = localCriterion.backward(output, target)
                localModel.backward(input, errors)
                val trainGrad = localModel.getParametersTable()[Table](layerName)[Tensor[T]](gradName)
                val gsum = preGrad.sub(trainGrad).abs().sum().asInstanceOf[Float]
                trainMeanAry(i) += gsum // get gradient
              }
          ...
          trainTimeCost += (System.nanoTime() - startTraining)
          ...
          while (i < finishedThreads.size) {
            ...
            compMeanSum += compMeanAry(finishedThreads(i))
            trainMeanSum += trainMeanAry(finishedThreads(i))
            i += 1
          }
      ...
      if (dropPercentage == 0.0 ||
        ...
        val iterationdelratio: Float = iterationDelNum.value * 1.0f / recordsNum.value
        val timeratio: Float = compTimeCost.value * 1.0f / trainTimeCost.value
        val compmean: Float = compMeanSum.value
        val trainmean: Float = trainMeanSum.value
        logger.info(s"${_header} Iterationdelratio: ${iterationdelratio} Timeratio: ${timeratio} CompMean: ${compmean} TrainMean: ${trainmean} " +
          s"compTime: ${compTimeCost.value * 1.0 / 1e9} trainTime: ${trainTimeCost.value * 1.0 / 1e9}")
        driverState("Iterationdelratio") = iterationdelratio
        driverState("Timeratio") = timeratio
        driverState("CompMean") = compmean
        driverState("TrainMean") = trainmean

        if(curEpoch > notEpoch && notIteration > 0 && curIteration % notIteration == 0){
