...
object Utils {
  case class TrainParams(
                          ...
                          epochstep: Int = 20,
                          epochgamma: Double = 0.375,
                          notEpoch: Int = 5,
                          hasbn: Boolean = false)

  val trainParser = new OptionParser[TrainParams]("Alexnet") {
    head("Train alexNet model")
    opt[String]("appName")
    ...
    opt[Int]("maxEpoch")
      .text("number of epochs; default is 165")
      .action((x, c) => c.copy(maxEpoch = x))
    opt[Int]("maxIteration")
      .text("number of iteration default is 100000")
      .action((x,c) => c.copy(maxIteration = x))
    opt[Double]("learningRate")
      .text("initial learning rate; default is 0.1")
      .action((x, c) => c.copy(learningRate = x))
    opt[Double]("learningRateDecay")
      .text("initial learningDecay rate ; default is 0.0")
    ...
    opt[Double]("weightDecay")
      .text("weightDecay; default is 1e-4")
    ...
    opt[Int]("epochstep")
      .text("epochstep")
      .action((x,c) => c.copy(epochstep = x))
    opt[Double]("epochgamma")
      .text("epochgamma")
      .action((x,c) => c.copy(epochgamma = x))
    opt[Int]("notEpoch")
      .text("not del epoch")
