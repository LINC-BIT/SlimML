...
//      s.add(Sbn(n))
    ...
    if (dataSet == DatasetType.ImageNet) {
      ...
      model.add(Convolution(3, 64, 7, 7, 2, 2, 3, 3, optnet = optnet, propagateBack = true).setName("conv_1"))
        .add(Sbn(64))
        .add(ReLU(true))
      ...
      model.add(Convolution(3, 16, 3, 3, 1, 1, 1, 1, optnet = optnet, propagateBack = true).setName("conv_1"))
